...
/// A set of bytes that are said to never be part of any match.
///
...
///
/// `Matcher` implementations that produce this set must never report false
/// positives, but may produce false negatives. That is, is a byte is in this
/// set then it must be guaranteed that it is never in a match. But, if a byte
/// is not in this set, then callers cannot assume that a match exists with
/// that byte.
#[derive(Clone, Debug)]
pub struct NonMatchingBytes(BitSet);
...
impl fmt::Debug for BitSet {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        for b in (0..256).map(|b| b as u8) {
            if NonMatchingBytes(*self).contains(b) {
                fmtd.entry(&b);
            }
        }
        fmtd.finish()
...
impl NonMatchingBytes {
    /// Create an empty set of non-matching bytes such that every byte is said
    /// to possibly participate in a match.
    pub fn empty() -> NonMatchingBytes {
        NonMatchingBytes(BitSet([0; 4]))
    }

    /// Create an full set of non-matching bytes such that every byte is said
    /// to never participate in a match.
    pub fn full() -> NonMatchingBytes {
        NonMatchingBytes(BitSet([u64::MAX; 4]))
    }
...
pub trait Matcher {
    ...
    fn non_matching_bytes(&self) -> Option<&NonMatchingBytes> {
        None
    }
