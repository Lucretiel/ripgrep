...
fn example() -> Result<(), Box<Error>> {
    ...
    Searcher::new().search_slice(&matcher, SHERLOCK, UTF8(|lnum, line| {
        // A RegexMatcher never returns an error, and we are guaranteed to
        // find a match, so the double unwrap is OK. Other implementations of
        // `Matcher` may return errors though!
